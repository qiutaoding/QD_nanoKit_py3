...
library("ggplot2")
library("seqinr")
df=read.table("plot/PD1074_5s.csv",header=FALSE)
blast_title<-c("query","subject","ldentity","length","mismatch","gap","qstart","qend","sstart","send","evalue","bitscore")
colnames(df)<-blast_title
fa<-read.fasta(file="extracted_read/PD1074_read.fa")
readlist <-read.table("extracted_read/PD1074_list")
readplot <-function(list_element){
  ...
  if(nrow(df_left_8)>0){
    p<-p+geom_segment(data=df_left_8,mapping=aes(x=sstart,y=qstart,xend=send,yend=qend),
                      ...
                      linejoin='mitre')}
  fileName <-substring(readName,8)
  if ((nrow(df_left_14) >0 |nrow(df_left_8) >0 )& nrow(df_5s_1) ==0&nrow(df_5s_2) ==0){
    pdf(paste0("plot/PD1074/left_only/",fileName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off()  
  }else if ((nrow(df_left_14) >0 |nrow(df_left_8) >0 )& nrow(df_5s_2) > 0){
    pdf(paste0("plot/PD1074/left_cross/",fileName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off()
  }else if (nrow(df_5s_1)>0 & nrow(df_5s_2) > 0){
    pdf(paste0("plot/PD1074/variant/",fileName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off()
  } else if ((nrow(df_left_14) >0 |nrow(df_left_8) > 0 |nrow(df_1st_gap) > 0 ) & (nrow(df_5s_1) > 0|nrow(df_5s_2) > 0)){
    pdf(paste0("plot/PD1074/left/",fileName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off()  
  }else if (nrow(df_right)>0 &(nrow(df_5s_1)>0|nrow(df_5s_2) > 0)){
    pdf(paste0("plot/PD1074/right/",fileName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else if (nrow(df_right)>0 &(nrow(df_5s_1)==0|nrow(df_5s_2) == 0)){
    pdf(paste0("plot/PD1074/right_only/",fileName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else if (nrow(df_transition)>0 &(nrow(df_5s_1)>0|nrow(df_5s_2) > 0)){
    pdf(paste0("plot/PD1074/transition/",fileName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else if (nrow(df_5s_1)>0 & nrow(df_5s_2) == 0){
    pdf(paste0("plot/PD1074/5s_only/",fileName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else{}
}
