...
library("ggplot2")
library("seqinr")
df=read.table("plot/PD1074_pac_5s.csv",header=FALSE)
blast_title<-c("query","subject","ldentity","length","mismatch","gap","qstart","qend","sstart","send","evalue","bitscore")
colnames(df)<-blast_title
fa<-read.fasta(file="extracted_read/pac_5s_read.fa")
readlist <-read.table("extracted_read/pac_5s_list")
readplot <-function(list_element){
  ...
  if ((nrow(df_left_14) >0 |nrow(df_left_8) >0 )& nrow(df_5s_1) ==0&nrow(df_5s_2) ==0){
    pdf(paste0("plot/PD1074_pac/left_only/",readName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off()  
  }else if ((nrow(df_left_14) >0 |nrow(df_left_8) >0 )& nrow(df_5s_2) > 0){
    pdf(paste0("plot/PD1074_pac/left_cross/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off()
  }else if (nrow(df_5s_1)>0 & nrow(df_5s_2) > 0){
    pdf(paste0("plot/PD1074_pac/variant/",readName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off()
  } else if ((nrow(df_left_14) >0 |nrow(df_left_8) > 0 |nrow(df_1st_gap) > 0 ) & (nrow(df_5s_1) > 0|nrow(df_5s_2) > 0)){
    pdf(paste0("plot/PD1074_pac/left/",readName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off()  
  }else if (nrow(df_right)>0 &(nrow(df_5s_1)>0|nrow(df_5s_2) > 0)){
    pdf(paste0("plot/PD1074_pac/right/",readName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else if (nrow(df_right)>0 &(nrow(df_5s_1)==0|nrow(df_5s_2) == 0)){
    pdf(paste0("plot/PD1074_pac/right_only/",readName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else if (nrow(df_transition)>0 &(nrow(df_5s_1)>0|nrow(df_5s_2) > 0)){
    pdf(paste0("plot/PD1074_pac/transition/",readName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else if (nrow(df_5s_1)>0 & nrow(df_5s_2) == 0){
    pdf(paste0("plot/PD1074_pac/5s_only/",readName,".pdf"),width=5+readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else{}
}
