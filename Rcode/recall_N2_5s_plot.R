...
library("ggplot2")
library("seqinr")
df=read.table("plot/N2L1_5s.csv",header=FALSE)
blast_title<-c("query","subject","ldentity","length","mismatch","gap","qstart","qend","sstart","send","evalue","bitscore")
colnames(df)<-blast_title
fa<-read.fasta(file="extracted_read/N2L1_read.fa")
readlist <-read.table("extracted_read/N2L1_list")
readplot <-function(list_element){
  ...
  if ((nrow(df_left_14) >0 |nrow(df_left_8) >0 )& nrow(df_5s_1) ==0 & nrow(df_5s_2) ==0){
    pdf(paste0("plot/N2L1/left_only/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off()  
  }else if ((nrow(df_left_14) >0 |nrow(df_left_8) >0 )& nrow(df_5s_2) > 0){
    pdf(paste0("plot/N2L1/left_cross/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off()
  }else if (nrow(df_5s_1)>0 & nrow(df_5s_2) > 0){
    pdf(paste0("plot/N2L1/variant/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off()
  } else if ((nrow(df_left_14) >0 |nrow(df_left_8) > 0 |nrow(df_1st_gap) > 0 ) & (nrow(df_5s_1) > 0|nrow(df_5s_2) > 0)){
    pdf(paste0("plot/N2L1/left/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off()  
  }else if (nrow(df_right)>0 &(nrow(df_5s_1)>0|nrow(df_5s_2) > 0)){
    pdf(paste0("plot/N2L1/right/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else if (nrow(df_right)>0 &(nrow(df_5s_1)==0|nrow(df_5s_2) == 0)){
    pdf(paste0("plot/N2L1/right_only/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else if (nrow(df_transition)>0 &(nrow(df_5s_1)>0|nrow(df_5s_2) > 0)){
    pdf(paste0("plot/N2L1/transition/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else if (nrow(df_5s_1)>0 & nrow(df_5s_2) == 0){
    pdf(paste0("plot/N2L1/5s_only/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off() 
  }else{}
}
