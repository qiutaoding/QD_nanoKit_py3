...
library("ggplot2")
library("seqinr")
df=read.table("plot/zzy0653_45s_plot.csv",header=FALSE)
blast_title<-c("query","subject","ldentity","length","mismatch","gap","qstart","qend","sstart","send","evalue","bitscore")
colnames(df)<-blast_title
fa<-read.fasta(file="extracted_read/zzy0653_45s_read.fa")
readlist <-read.table("extracted_read/zzy0653_45s_list")
readplot <- function(list_element){
  ...
  if (nrow(df_mos)>0  ){
    pdf(paste0("plot/zzy0653/mos/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off()  
  }else if (nrow(df_left)>0 & nrow(df_26s) >0 ){
    pdf(paste0("plot/zzy0653/left/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off()  
  }else if (nrow(df_telo)>10 & nrow(df_18s)>2){
    pdf(paste0("plot/zzy0653/telo/",readName,".pdf"),width=readLength/1000,height = calheight)
    print(p)
    dev.off()  
  }else{}
}
